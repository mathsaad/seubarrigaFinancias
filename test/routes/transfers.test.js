...
test('Deve inserir uma transferencia com sucesso', () => {
  return request(app).post(MAIN_ROUTE)
    .set('authorization', `bearer ${TOKEN}`)
    .send({ description: 'Regular transfer', user_id: 1000000, acc_ori_id: 1000000, acc_dest_id: 1000001, ammount: 100, date: new Date() })
    .then(async (res) => {
...
describe('Ao salvar uma transferencia inválida...', () => {
  const validTransfer = { description: 'Regular transfer', user_id: 1000000, acc_ori_id: 1000000, acc_dest_id: 1000001, ammount: 100, date: new Date() };
  ...
  test('Não deve inserir sem decrição', () => {
  ...
  test('Não deve inserir sem conta de origem', () => {
    testTemplate({ acc_ori_id: null }, 'Conta de origem é um atributo obrigatório')
  });
  test('Não deve inserir sem conta de destino', () => {
    testTemplate({ acc_dest_id: null }, 'Conta de destino é um atributo obrigatório')
  });
  test('Não deve inserir se as contas de origem e destino forem as mesmas', () => {
    testTemplate({ acc_dest_id: 1000000 }, 'Não é possível transferir de uma conta para ela mesma')
  });
  test('Não deve inserir se as contas pertencerem a outro usuário', () => {
    testTemplate({ acc_ori_id: 1000002 }, 'Conta #1000002 não pertence ao usuário')
  });
